        ...
        <v-list class="list" flat>
          ...
          <v-list-item-group>
            ...
            <v-list-item v-for="location in locations" :key="location.name" link>
              ...
              <v-list-item-title @click="clickMarker(location.id - 1, {
                  lat: location.lat,
                  lng: location.lng,
                })
                ">
          ...
          <hr>
          <h3 class="h3search">Search For Place To Add To Your Favorite List</h3>
          <GMapAutocomplete class="inputs in1" placeholder="Search" :options="{
              types: ['establishment'],
              componentRestrictions: {
                country: 'PS',
              },
            }" @place_changed="(event) => onPlaceSelected(event.geometry.location)" /><br />
        </v-list>
      </v-navigation-drawer>
